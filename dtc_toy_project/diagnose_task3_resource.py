...
def allocate_resources(scenario_id, patients, resources):
    ...
    for i, (p, sev) in enumerate(scored):
        ...
        if i >= n_get_care:
            continue  # no assignment at all -- matches the "empty assignment" pattern
                      # seen in ground truth for lower-severity patients

        res_list = ["hospital_bed"]  # not gated by a literal per-patient cap, see docstring
    ...
    return {
        "case_id": scenario_id,
        "evacuated_patients": [],  # no reliable evacuation signal found in available features
